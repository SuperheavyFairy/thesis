...
A key objective of this thesis is to improve the topology of meshes generated from implicit representations. To achieve this goal, a source of high-quality topology information is required. ShapeNetCore~\cite{shapenet2015} is selected for this purpose because it contains a large collection of manually created CAD models that have been designed according to established modeling practices.
...
An implicit surface is defined as the set of points satisfying $f(\mathbf{x}) = c$ where $f(\mathbf{x})$ is a scalar-valued function and $c$ is a predefined isovalue. The resulting surface corresponds to the level set of the function. In many applications, particularly Signed Distance Fields (SDFs), the surface is represented by the zero-level set where $c=0$.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Figure/Figure2-ImplicitExplicit.png}
	\caption{Comparison of implicit and explicit geometric representations.}
