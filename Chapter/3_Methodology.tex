...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{Figure/Figure3-Summary}
	\caption{Overview of the proposed topology-aware mesh generation framework. An input image is processed by TRELLIS to produce a sparse tensor representation, which is decoded by FlexiCubes to generate an initial mesh. A point cloud sampled from the generated mesh is analyzed by a pretrained Transformer to predict surface normals and tangent directions. These predictions are used as topology supervision during the optimization of the sparse features, resulting in a topology-aware mesh with improved edge-flow while preserving geometric fidelity.}
	\label{fig:figure3-overview}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Surface sampling on a triangular mesh.}
...
\begin{figure}[H]
	...
	\caption{Principal direction of a quadrilateral mesh.}
...
This representation allows the network to learn both the local surface orientation and the tangent direction simultaneously.
...
where $\phi:\mathbb{R}^3\rightarrow\mathbb{R}^d$ denotes the learnable feature embedding.
...
whereas the predicted angle is constrained to $[-\pi,\pi]$ using the hyperbolic tangent activation $\hat{\theta}=\pi\tanh(x)$

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{"Figure/Figure33-2 Artchitecture"}
	\caption{Pipeline of the proposed Transformer-based point cloud network. Input points are embedded into feature vectors, processed by stacked Transformer encoder layers, and decoded by a prediction head to predict a unit surface normal and tangent direction angle for every point.}
...
The pretrained transformer described in Section~\ref{sec:extract_topology} encodes this reference point cloud once to produce a set of latent feature embeddings,
